...
export const WeekView: React.FC<WeekViewProps> = ({
  ...
  return (
    <div>
      ...
      <div className="print:hidden space-y-4 pb-20">
        {/* Header Controls */}
      ...
      {/* Print-Only Template (Simple, High-Contrast & Beautifully Structured Vertical Agenda) */}
      <div className="hidden print:block font-mono text-black p-4 space-y-6">
        {/* Print Header */}
        ...
        <div className="space-y-6">
          {activeDays.map((day) => {
            ...
            const sortedSessions = [...daySessions].sort((a, b) => {
              const slotA = slots.find((ts) => ts.id === a.slotId);
              const slotB = slots.find((ts) => ts.id === b.slotId);
              return (slotA?.startMinutes || 0) - (slotB?.startMinutes || 0);
            });

            return (
              <div key={day} className="border border-black p-4 space-y-3 page-break-inside-avoid">
                <div className="flex justify-between items-center border-b border-black pb-1.5">
                  <h2 className="text-sm font-bold uppercase tracking-wider">{day}</h2>
                  <span className="text-xs font-bold text-slate-700">{getCalendarDateForDay(day)}</span>
                </div>

                {sortedSessions.length > 0 ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {sortedSessions.map((session) => {
                      const slot = slots.find((ts) => ts.id === session.slotId);
                      return (
                        <div key={session.id} className="text-xs space-y-1.5 p-3 bg-slate-50 border border-slate-300">
                          <div className="flex justify-between items-start font-bold border-b border-dashed border-slate-300 pb-1">
                            <span className="text-black text-sm">{session.courseCode}</span>
                            <span className="bg-black text-white px-1.5 py-0.5">Rm {session.room}</span>
                          </div>
                          <div className="font-extrabold text-black">{session.courseTitle}</div>
                          <div className="text-slate-800"><span className="font-bold">Instructor:</span> {session.teacher}</div>
                          {slot && (
        ...
        {/* Course Reference & Teachers */}
        <div className="border-t-2 border-black pt-4 space-y-3 page-break-inside-avoid">
          <h3 className="text-xs font-bold uppercase tracking-wider">Course Reference:</h3>
          <div className="grid grid-cols-2 gap-3 text-[11px]">
            {Object.values(COURSES).map((c) => (
              <div key={c.code} className="border border-slate-300 p-2.5 bg-slate-50">
                <span className="font-bold text-black border-b border-black pb-0.5 inline-block mb-1">{c.code}: {c.title}</span>
                <div className="text-slate-800"><span className="font-bold">Instructor:</span> {c.teacher}</div>
                <div className="text-[10px] text-slate-500 mt-0.5 italic">{c.description}</div>
              </div>
            ))}
          </div>
        </div>
